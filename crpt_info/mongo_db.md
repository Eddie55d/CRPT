...
```js
cfg = rs.conf();
cfg.members[<index_node>].priority = 1;  // Установите приоритет
cfg.members[<index_node>].votes = 1;      // Установите голоса
cfg.members[<index_node>].hidden = false; // Установите видимость
rs.reconfig(cfg);
```
Замените idx_node на индекс новой ноды
...
Параметры голосования могут быть следующими:

- 1: Узел участвует в выборах и имеет один голос.
- 0: Узел не участвует в выборах и не имеет голоса. Это полезно для узлов, которые вы хотите использовать только как резервные, например, арбитры (arbiter).
...
**Initial sync** в MongoDB используется для устранения фрагментации данных. Во время этого процесса вторичный узел делает начальный снимок данных первичного узла, копируя все базы данных, коллекции и индексы и сохраняя данные в упорядоченных блоках. Это помогает удалить фрагментацию и оптимизировать работу базы данных. 
